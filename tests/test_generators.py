...
def test_filter_by_currency(list_for_generator: list, empty_lsts: list) -> None:
    """Функция тестирует filter_by_currency from src.generators"""
    usd_transactions_with_currency = filter_by_currency(list_for_generator, 'USD')
    ...
    with pytest.raises(StopIteration):
        usd_transactions_wo_currency = filter_by_currency(list_for_generator, '')
        assert next(usd_transactions_wo_currency)

    with pytest.raises(StopIteration):
        usd_transactions_wo_currency = filter_by_currency(list_for_generator, 'EUR')
        assert next(usd_transactions_wo_currency)
...
def test_transaction_descriptions(list_for_generator: list, empty_lsts: list) -> None:
    """Функция тестирует transaction_descriptions from src.generators"""
    descriptions = transaction_descriptions(list_for_generator)
